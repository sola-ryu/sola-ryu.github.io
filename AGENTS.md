...
- **Image generation:** Use the comfyui tool for cover images and illustrations. **Always spawn a subagent with a long timeout** (at least 5 minutes, much longer for multiple images) — ComfyUI is local and can be slow, in-session exec calls get SIGTERM'd mid-wait. Save outputs to `src/assets/blog-images/` before referencing in posts.
